<script lang="ts">
    ...
    let availableProviders: string[] = ['openai', 'gemini', 'anthropic'];
    let showFilters = false;

    let filteredModels: string[] = [];
  
    onMount(() => {
      availableModels = Array.from(new Set(data.map(d => d.model)));
    ...
    function updateFilteredModels() {
      ...
      if (providers.length === 0) {
        ...
        filteredModels = providers.reduce((acc: string[], provider: string) => {
          ...
          return acc.concat(providerModels.filter(model => availableModels.includes(model)));
        }, []);
      }
    ...
    <div class="filters-section">
      ...
      <div class="filters" class:show={showFilters}>
        ...
        <div class="filter-group half-width">
          ...
          <div class="multi-select">
            <div class="selected-pills">
              {#each (filters.providers ?? []) as provider (provider)}
        ...
        <div class="filter-group half-width">
          ...
          <div class="multi-select">
            <div class="selected-pills">
              {#each (filters.models ?? []) as model (model)}
